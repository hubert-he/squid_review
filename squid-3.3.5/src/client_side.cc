...
parseHttpRequest(ConnStateData *csd, HttpParser *hp, HttpRequestMethod * method_p, HttpVersion *http_ver)
{
    ...
    *method_p = HttpRequestMethod(&hp->buf[hp->req.m_start], &hp->buf[hp->req.m_end]+1);
    ...
    tempBuffer.data = result->reqbuf; // StroeIOBuffer
    tempBuffer.length = HTTP_REQBUF_SZ;
...
clientProcessRequest(ConnStateData *conn, HttpParser *hp, ClientSocketContext *context, const HttpRequestMethod& method, HttpVersion http_ver)
{
    ...
    if (context->flags.parsed_ok == 0) { // parse Failed
        clientStreamNode *node = context->getClientReplyContext();
...
//					CommCbMemFunT<ConnStateData, CommCloseCbParams>(CbcPointer<ConnStateData>(result), &ConnStateData::connStateClosed))
    comm_add_close_handler(client->fd, call);

    if (Config.onoff.log_fqdn)
        fqdncache_gethostbyaddr(client->remote, FQDN_LOOKUP_IF_MISS);

#if USE_IDENT  //  Authentication Server
...
clientHttpConnectionsOpen(void)
{
    ...
    for (s = Config.Sockaddr.http; s; s = s->next) {
		...
		// sub will be called by Comm::TcpAcceptor::notify()
        AsyncCall::Pointer listenCall = asyncCall(33,2, "clientListenerConnectionOpened",
                                        ListeningStartedDialer(&clientListenerConnectionOpened, s, Ipc::fdnHttpSocket, sub));
		// AsyncCallQueue::Instance().schedule(listenCall);  just appending the call-func into the queue.
...
static void
clientListenerConnectionOpened(AnyP::PortCfg *s, 
