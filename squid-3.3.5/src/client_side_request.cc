...
ClientHttpRequest::httpStart()
{
    ...
    clientStreamRead(node, this, node->readBuffer); // call clientGetMoreData()
    PROF_stop(httpStart);
}
...
ClientHttpRequest::doCallouts()
{
    ...
    if (calloutContext->error) {
        ...
        StoreEntry *e= storeCreateEntry(uri, uri, request->flags, request->method); //  Entry store
#if USE_SSL
        if (sslBumpNeeded()) {
...
#endif
        {
            ...
            clientStreamNode *node = (clientStreamNode *)client_stream.tail->prev->data; // newServer
            clientReplyContext *repContext = dynamic_cast<clientReplyContext *>(node->data.getRaw());// newServer
            assert (repContext);
