...
// C = cbcpointer<connstatedata>
// CommCbMemFunT<ConnStateData, CommIoCbParams>(CbcPointer<ConnStateData>, &ConnStateData::clientReadRequest)
// CommCbMemFunT<ConnStateData, CommCloseCbParams>(CbcPointer<ConnStateData>(result), &ConnStateData::connStateClosed)
#if  xxx
class CommDialerParamsT
{
public:
    CommDialerParamsT(const CommCloseCbParams &io): CommCloseCbParams(io) {}
...
public:
    ...
    CommCbMemFunT(const CbcPointer<C> &job, Method meth): JobDialer<C>(job),
            CommDialerParamsT<Params_>(job.get()), // job.get() ==> ConnStateData *
            method(meth) {}
